...
#     checks if the URI is valid (by requesting it), and if so, stores the
...
def CheckURI(uri, timeout=5):
    # The timeout parameter prevents the Requests from waiting for too long 
...
class Shortener(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if name:
            if name in memory:
                # If the name has already stored in the dictionary, redirect directly 
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Missing Field(s). ".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("Failed to reach the url '{}'".format(longuri).encode())
